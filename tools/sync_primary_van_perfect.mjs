...
async function run() {
    ...
    // Global STT corresponds to row index - 3
    let globalSTT = 1;
    for (let i = 4; i < rows.length; i++) {
        ...
        if (isPrimary) {
            ...
            updates.push({ stt: globalSTT.toString(), ho, ten, lop, van: valV });
        }
        
        globalSTT++;
    }

    console.log(`Found ${updates.length} primary students. Syncing VĂN strictly by STT and Name...`);
    ...
    for (const update of updates) {
        try {
            let { data: students, error: selectError } = await supabase
                ...
                .eq('STT', update.stt); // Use EXACT STT matching
            ...
            if (!students || students.length === 0) {
                continue; // Cannot find this STT in Supabase
            }

            const targetStudent = students[0]; 

            // Verify it's actually the same student
    ...
    console.log(`Sync complete! Updated ${successCount} differing records using precise STT + Name matching.`);
    process.exit(0);
}
